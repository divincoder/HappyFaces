...
class FaceDetector(private val callback: DetectorCallback?) : IFrameProcessor {
    ...
    interface DetectorCallback {
        fun onSuccess(frameData: ByteBuffer, results: List<FirebaseVisionFace>, frameMetadata: FrameMetadata, firebaseFrameMetadata: FirebaseVisionImageMetadata)
    ...
    private fun processImage(data: ByteBuffer, frameMetadata: FrameMetadata) {
        ...
        delegateDetector.detectInImage(image)
            .addOnSuccessListener { results ->
                callback?.onSuccess(data, results, frameMetadata, metadata)
                processLatestImage()
            }
